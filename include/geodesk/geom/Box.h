...
public:
	...
	double maxLat() const { return Mercator::latFromY(maxY()); }
	Coordinate topLeft() const { return {minX(), maxY()}; }
	Coordinate bottomRight() const { return {maxX(), minY()}; }
	Coordinate topRight() const { return {maxX(), maxY()}; }
	Coordinate bottomLeft() const { return {minX(), minY()}; }
...
private:
	 ...
	 */
	static int trimmedSubtract(int32_t x, int32_t y)
	{
		int r = x - y;
